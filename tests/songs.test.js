...
describe("routes/songs", () => {
  ...
  it("should throw an error if POST /songs name and artist value are invalid ", async () => {
    const response = await request(app).post("/songs");

    expect(response.status).toEqual(400);
    expect(response.body.message).toEqual(
      "Name and artist value are not valid inputs"
    );
  });
});
