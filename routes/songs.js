...
router.get("/", async (req, res, next) => {
  // can add catch got getSongs() in case it fails
  const response = await getSongs().catch(e => next(e));
...
router.post("/", async (req, res, next) => {
  ...
  const response = await createSong(req.body).catch(e => next(e));
...
router.put("/:id", async (req, res, next) => {
  ...
  const response = await updateSong(req.body, req.song).catch(e => next(e));
  res.status(200).json(response);
});

//delete a song with id, and return deleted song
router.delete("/:id", async (req, res, next) => {
  const response = await deleteSong(req.song).catch(e => next(e));
  res.status(200).json(response);
});
